...
- Name: Practice\9thMayiFrame.fixed.spec.ts >> Validate iFrame - fixed
- Location: tests\Practice\9thMayiFrame.fixed.spec.ts:3:5
...
Locator: locator('iframe[id*=\'frame-one\']').contentFrame().getByText('Submitted form data')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('iframe[id*=\'frame-one\']').contentFrame().getByText('Submitted form data')
                ...
                - generic [ref=f1e7]:
                  ...
                  - generic [ref=f1e18]:
                    ...
                    - generic [ref=f1e22]:
                      ...
                      - spinbutton "Year" [ref=f1e24]: "2019"
                  - generic [ref=f1e25]:
                    - generic [ref=f1e26]: Notes
                    - textbox "Notes" [ref=f1e27]:
                      - /placeholder: Any notes about the registration
                      - text: This is my dream car
                  - generic [ref=f1e28]:
                    - button "Submit registration" [active] [ref=f1e29] [cursor=pointer]
                    - button "Reset" [ref=f1e30] [cursor=pointer]
                  - generic [ref=f1e31]: "{ \"vehicleName\": \"BMW\", \"ownerName\": \"Pallishree\", \"regNumber\": \"KA-01-PS-63339\", \"vehicleType\": \"SUV\", \"year\": \"2019\", \"notes\": \"This is my dream car\" }"
            - generic [ref=e251]:
              - generic [ref=e252]:
                - generic [ref=e253]: id
                - text: =frame-one
...
```ts
  ...
  2  | 
  3  | test('Validate iFrame - fixed', async ({ page }) => {
  4  |   await page.goto('https://app.thetestingacademy.com/playwright/frames/');
  5  | 
  6  |   const frame = page.frameLocator("iframe[id*='frame-one']");
  7  | 
  8  |   await frame.getByRole('textbox', { name: 'Vehicle name' }).fill('BMW');
  9  |   await frame.getByRole('textbox', { name: 'Owner name' }).fill('Pallishree');
  10 |   await frame.getByRole('textbox', { name: 'Registration number' }).fill('KA-01-PS-63339');
  11 |   await frame.getByRole('combobox', { name: 'Vehicle type' }).selectOption('SUV');
  12 |   await frame.getByRole('spinbutton', { name: 'Year' }).fill('2019');
  13 |   await frame.getByRole('textbox', { name: 'Notes' }).fill('This is my dream car');
  14 | 
  15 |   await frame.getByRole('button', { name: 'Submit registration' }).click();
  16 | 
  17 |   // Verify submit action produced a success/result text inside iframe.
> 18 |   await expect(frame.getByText('Submitted form data')).toBeVisible();
     |                                                        ^ Error: expect(locator).toBeVisible() failed
  19 | });
  20 | 
```