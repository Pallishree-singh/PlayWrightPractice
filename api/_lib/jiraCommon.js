...
export function validateJiraRequest(body) {
...
export async function fetchJiraIssue(config) {
  ...
  return {
    ...
    description: extractJiraText(fields.description || issue.description),
    acceptanceCriteria: fields.customfield_12345 || issue.acceptanceCriteria || "",
    priority: fields.priority?.name || issue.priority || "",
    labels: Array.isArray(fields.labels) ? fields.labels : issue.labels || [],
...
export async function generateMarkdownTestPlan(config, issue) {
  const systemPrompt = [
...
export async function testJiraAccess(config) {
  const authStrategies = buildAuthStrategies(config);
  const issueKey = encodeURIComponent(config.jiraIssueKey);
...
export function buildManualIssue(body, parsed) {
