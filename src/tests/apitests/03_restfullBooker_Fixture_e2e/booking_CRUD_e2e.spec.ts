...
import { visualStep } from '@utils/VisualScript';
...
test.describe.serial('@e2e @P0 Level 3 - Booking lifecycle (token from fixture)', () => {
    ...
    test('create a booking', async ({ bookingApi, page }, testInfo) => {
        const payload = buildBooking({ firstname: 'E2E', lastname: 'Journey' });

        await visualStep(page, 'POST /booking with a generated payload', async () => {
    ...
    test('update the booking (token comes from the fixture)', async ({
        ...
        await visualStep(page, 'PUT /booking/{id} with the fixture token', async () => {
        ...
        await visualStep(page, 'GET /booking/{id} to confirm the change persisted', async () => {
    ...
    test('delete the booking and confirm it is gone', async ({ bookingApi, bookerToken, page }) => {
        await visualStep(page, 'DELETE /booking/{id} with the fixture token', async () => {
        ...
        await visualStep(page, 'GET /booking/{id} should now 404', async () => {
